...
box_O = boxplot(Temp_O,'colors','r','positions',position_O,'width',0.16,'sym',' ','whisker',1.5);hold on;
...
box_Z = boxplot(Temp_Z,'colors','b','positions',position_Z,'width',0.16,'sym',' ','whisker',1.5);  
...
box_S = boxplot(Temp_S,'colors','g','positions',position_S,'width',0.16,'sym',' ','whisker',1.5);

% legend('MDS-MAP','MDS-MAP(P)','SMDS','SMDS(P)','GPS');
box_vars=findall(gca,'Tag','Box');
hLegend = legend(box_vars([15,1,10]), {'GPS','Classical MDS','Optimized MDS'});

xlabel('$k$','interpreter','latex','fontsize',16);% M
ylabel('Error (m)');
set(gca,'XLim',[0.8 6]);
set(gca,'YLim',[0 0.6]);
