...
clear;clc;close all;
load data2;
cc=real(cmds);ww=real(wmds);gg=real(gps);

c=[cc(:,5),cc(:,4),cc(:,3),cc(:,2),cc(:,1);];
w=[ww(:,5),ww(:,4),ww(:,3),ww(:,2),ww(:,1);];
g=[gg(:,5),gg(:,4),gg(:,3),gg(:,2),gg(:,1);];

Jan_O=g(:,1);Feb_O=g(:,2);Mar_O=g(:,3);Apr_O=g(:,4);May_O=g(:,5);
%Jun_O=g(:,6);Jul_O=g(:,7);Aug_0=g(:,8);
Temp_O = [Jan_O, Feb_O, Mar_O, Apr_O, May_O];
position_O = 1.18:1:5.18; 
% Define position for 12 Month_O boxplots 
box_O = boxplot(Temp_O,'colors','r','positions',position_O,'width',0.16,'sym',' ','whisker',2);hold on;
...
position_Z = 1.54:1:5.54;  % Define position for 12 Month_S boxplots 
box_Z = boxplot(Temp_Z,'colors','b','positions',position_Z,'width',0.16,'sym',' ','whisker',2);  

Jan_S=c(:,1);Feb_S=c(:,2);Mar_S=c(:,3);Apr_S=c(:,4);May_S=c(:,5);
%Jun_S=c(:,6);Jul_S=c(:,7);Aug_S=c(:,8);
Temp_S = [Jan_S, Feb_S, Mar_S, Apr_S, May_S];
position_S = 1.36:1:5.36;  % Define position for 12 Month_S boxplots 
box_S = boxplot(Temp_S,'colors','g','positions',position_S,'width',0.16,'sym',' ','whisker',2);

% legend('MDS-MAP','MDS-MAP(P)','SMDS','SMDS(P)','GPS');
box_vars=findall(gca,'Tag','Box');
hLegend = legend(box_vars([15,1,10]), {'GPS','Classical MDS','Optimized MDS'});

xlabel('$3\sigma_{g}$(m)','interpreter','latex','fontsize',16);% M
ylabel('Error (m)');
set(gca,'XLim',[0.8 6]);
set(gca,'YLim',[0 1]);
